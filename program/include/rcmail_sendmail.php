...
class rcmail_sendmail
{
    ...
    public function headers_input()
    {
        ...
        if($from!=null && $from!="")
        {
	        if (preg_match('/\p{Thai}/u', $from) === 1) {
		        $na=explode("@",$mailto);
		        if(getmxrr($na[1],$mx_details)){
				...
				$utf8_sup=0;
				$fp = fsockopen($mx, 25, $errno, $errstr, 30);
				if (!$fp) {
				    ...
				    while (!feof($fp)) {
				        //$output .= fgets($fp, 128);
				        if(preg_match("/SMTPUTF8/i",fgets($fp, 128))) {
				                $utf8_sup=1;
				        }
				    }
				...
				if($utf8_sup==0) {
					/*
					if($this->rcmail->user->data['user_id']==7)
		        	{
			        	return $this->options['error_handler']('Change to '.$tmp_test, 'warning');
		        	}
		        	*/
					...
					foreach ($first_iden as $idx => $ident) {
                        ...
                        if(preg_match('/^[ก-๙a-zA-Z0-9_+&*-]+(?:\.[ก-๙a-zA-Z0-9_+&*-]+)*@(?:[ก-๙a-zA-Z0-9-]+\.)+[ก-๙a-zA-Z]{2,16}$/u', $ident['email']) === 0 && $iden_set==0) {
							...
							if($from_string)
							{
								$from_string=str_replace($from,$ident['email'],$from_string);
							}
							$from=$ident['email'];
							$iden_set=1;
						}
		        	...
		        	//return $this->options['error_handler']('Change to '.$iden, 'warning');

			    }
			}
	        
	        //$get_iden = $this->rcmail->user->get_identity(8);
	        //return $this->options['error_handler']('a'.$get_iden['email'], 'warning');
	        //$user_id = $this->rcmail->user->data['user_id'];
			//return $this->options['error_handler']('a'.$user_id, 'warning');		
		}
		//end NA
        ...
        //$from_string="thanapat@kon.in.th";
    ...
    public function email_input_format($mailto, $count = false, $check = true)
    {
        ...
        foreach ($items as $item) {
            ...
            // address with name (handle name)
                ...
                $item     = $address;
            } else if(preg_match('/^[ก-๙a-zA-Z0-9_+&*-]+(?:\.[ก-๙a-zA-Z0-9_+&*-]+)*@(?:[ก-๙a-zA-Z0-9-]+\.)+[ก-๙a-zA-Z]{2,16}$/u', $item)) {
                // $item     = rcube_utils::idn_to_ascii($item);
                $item     = $item;
                $result[] = $item;
            }
